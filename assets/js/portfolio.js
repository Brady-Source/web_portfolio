...
(function () {
  ...
  function select(id, focusDetail) {
    ...
    for (var i = 0; i < buttons.length; i++) {
      buttons[i].setAttribute(
        "aria-current",
        buttons[i].getAttribute("data-id") === id ? "true" : "false",
      );
    }
    var proj = state.projects.filter(function (p) {
      return p.id === id;
    })[0];
  ...
  function buildMediaViewer(proj) {
    ...
    var isVideoLike = /\.(mp4|webm|ogg)$/.test(cleanSrc);
  ...
  function buildPdfViewer(proj) {
    ...
    var link = el("a", {
      ...
      attrs: { href: proj.pdf, target: "_blank", rel: "noopener" },
    });
  ...
  function buildDocViewer(proj) {
    ...
    var box = el("div", {
      ...
      attrs: {
        tabindex: "0",
        role: "document",
        "aria-label": proj.title + " — non-editable document summary",
      },
    });
    ...
    wrap.appendChild(
      el("p", {
        text: "Non-editable summary of the source document.",
        className: "doc-caption",
      }),
    );
    return wrap;
  ...
  function buildCodeDemoToolbar(proj, onSelect) {
    ...
    var codeLink = (proj.links || []).filter(function (l) {
      return l.type === "code";
    })[0];
    var demoLink = (proj.links || []).filter(function (l) {
      return l.type === "demo";
    })[0];

    var group = el("div", {
      className: "toggle-group",
      attrs: { role: "tablist", "aria-label": "View code or demo" },
    });
  ...
  function renderDetail(proj) {
    ...
    detailEl.appendChild(
      el("p", { text: proj.category, className: "detail__eyebrow" }),
    );
    detailEl.appendChild(
      el("h2", { text: proj.title, className: "detail__title" }),
    );
    if (proj.tagline)
      detailEl.appendChild(
        el("p", { text: proj.tagline, className: "detail__tagline" }),
      );
    if (proj.context)
      detailEl.appendChild(
        el("p", { text: proj.context, className: "detail__context" }),
      );

    // Toolbar (Code | Demo) only for projects with both a code and demo link.
    var hasCode = (proj.links || []).some(function (l) {
      return l.type === "code";
    });
    var hasDemo = (proj.links || []).some(function (l) {
      return l.type === "demo";
    });
    ...
    if (proj.viewer === "pdf" && proj.pdf) viewer = buildPdfViewer(proj);
      ...
      (proj.viewer === "image" ||
        proj.viewer === "gif" ||
        proj.viewer === "media") &&
      (proj.image || proj.gif || proj.media)
    ) {
      viewer = buildMediaViewer(proj);
    } else if (proj.viewer === "codeDemo")
      viewer = buildCodeDemoPlaceholder(proj);
    ...
    if (proj.description) {
      ...
      detailEl.appendChild(
        el("p", { text: proj.description, className: "detail__desc" }),
      );
    }
    ...
    if (proj.tech && proj.tech.length) {
      ...
      proj.tech.forEach(function (t) {
        tl.appendChild(el("li", { text: t, className: "tag" }));
      });
    ...
    if (proj.demoNote) {
      detailEl.appendChild(
        el("p", {
          text: proj.demoNote,
          className: "doc-caption doc-caption--flush",
        }),
      );
    }
  }
  ...
  function buildCodeDemoPlaceholder(proj) {
    ...
    var box = el("div", {
      ...
      attrs: {
        tabindex: "0",
        role: "document",
        "aria-label": proj.title + " overview",
      },
    });
    box.appendChild(
      el("p", {
        text: "This is a running full-stack application. Because its live features depend on a database and server, the interactive demo is disabled on this static portfolio.",
      }),
    );
    box.appendChild(
      el("p", {
  ...
  window.BF.loadJSON("assets/data/projects.json")
    .then(function (data) {
      state.projects = Array.isArray(data.projects) ? data.projects : [];
      renderList();
      // Deep-link support via hash
      var hash = (window.location.hash || "").replace("#", "");
      var initial = state.projects.filter(function (p) {
        return p.id === hash;
      })[0];
      if (initial) {
        state.filter = "all";
        ...
        select(state.projects[0].id, false);
      }
    })
    .catch(function (err) {
      detailEl.textContent = "";
      detailEl.appendChild(
        el("p", {
          text: "Sorry — the project list could not be loaded.",
          className: "prompt-empty",
        }),
      );
